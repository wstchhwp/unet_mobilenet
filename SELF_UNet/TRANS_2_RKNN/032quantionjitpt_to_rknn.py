...
if __name__ == '__main__':
    ...
    #     quantize_input_node=config_unet.quantization_on,
    #     # output_optimize=1,
    ...
    # Load pytorch model
    print('--> Loading model')
    ret = rknn.load_pytorch(model=config_unet.pt_jit_path, input_size_list=config_unet.input_size_list)
    if ret != 0:
    ...
    ret = rknn.build(do_quantization=False, pre_compile=config_unet.pre_compile)  #, dataset=config_unet.quantization_dataset)  # pre_compile=True,那么模型就只能在rk板子上运行
    if ret != 0:
