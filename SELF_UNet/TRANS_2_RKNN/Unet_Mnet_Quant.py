...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        # todo:22.7.5新增
        self.quant_res = torch.quantization.QuantStub()
        self.dequant_res = torch.quantization.DeQuantStub()
    ...
    def forward(self, x):
        ...
        if self.use_res_connect:
            x = self.add_residual.add(x, self.conv(x))
            #x = x + self.conv(x)
            return x
        else:
            return self.conv(x)
